...
Celem niniejszego laboratorium by³o zaprojektowanie, implementacja,
weryfikacja poprawnoœci dzia³ania oraz dobór parametrów algorytmów regulacji jednowymiarowego
procesu na grzej¹co-ch³odz¹cym stanowisku laboratoryjnym przedstawionym na rys \ref{Stanowisko grzewcze}.
...
\begin{enumerate}
	\label{Lista-zadan}
  \item Odczytaæ wartoœæ pomiaru temperatury dla termometru T1 dla mocy 26 \% grza³ki G1.
  \item Wyznaczyæ odpowiedŸ skokow¹ procesu dla trzech ró¿ych wartoœci G1.
  \item Wybraæ jedn¹ z odpowiedzi skokowych, przekszta³ciæ j¹ i wykorzystaæ w algorytmie DMC. 
  \item Zaimplementowaæ algorytm PID i DMC do regulacji procesu stanowiska w jêzyku MATLAB.
  \item Dobraæ nastawy algorytmu PID oraz parametry algorytmu DMC metod¹ eksperymentaln¹.
\end{enumerate}

\section{Punkt pracy stanowiska}
W pierwszej kolejnoœci sprawdzono mo¿liwoœæ sterowania i pomiaru w komunikacji ze stanowiskiem.
...
W tej czêœci laboratorium nale¿a³o przeprowadziæ eksperyment dla 3 ró¿nych wartoœci mocy grza³ki G1.
...
Patrz¹c na wykres odpowiedzi skokowych mo¿emy zauwa¿yæ, ¿e proces jest w przybli¿eniu liniowy. Na rys. \ref{charakterystyka-statyczna} zosta³a przedstawiona charakterystyka statyczna procesu.

\begin{figure}[h]
	\centering
	\includegraphics{../TikZ_rysunki/charakterystyka_stat.pdf}
	\caption{Charkterystyk statyczna procesu grzej¹co-ch³odz¹cego}
	\label{charakterystyka-statyczna}
\end{figure}


\section{OdpowiedŸ skokowa w algorytmie DMC}
Wykonanie tego zadania polega³o na przekszta³ceniu jednej z odpowiedzi
...
Do przekszta³cenia zebranej odpowiedzi skokowej, na tak¹ nadaj¹c¹ siê do algorytmu DMC wykorzystano program \emph{SkokDMC.m}.
...
\end{itemize}
W ten sposób przekszta³cona odpowiedŸ skokowa zosta³a zapisana do pliku \emph{dane1.mat}
i wykorzystana w dalszych czêœciach laboratoriów.
...
Nastêpnie u¿ywaj¹c skryptu \emph{Optymalizacja.m} zosta³y wyznaczone pozosta³e parametry transmitancji przybli¿aj¹cej odpowiedŸ skokow¹.
...
Kolejnym podpunktem zadañ laboratoryjnych by³o zaimplementowanie algorytmów regulacji PID oraz DMC w jêzyku MATLAB.
...
Ostatnim zadaniem by³ dobór nastawów obu algorytmów regulacji. W tym celu skorzystano z wczeœniej uzyskanej transmitancji aproksymuj¹cej skok procesu,
...
W dalszej kolejnoœci sprawdzane by³y jeszcze inne wartoœci parametrów w celu znalezienia jak najlepszych nastawów.
...
Z wykresów mo¿na wyczytaæ, ¿e zmiana parametrów PID poprawi³a jakoœæ regulacji. Co do regulatora DMC natomiast dzia³a on ca³kiem dobrze,
jednak zarówno PID jak i DMC s¹ nara¿one na zak³ócenia, które mo¿na zauwa¿yæ na wykresach.
Jednym z mo¿liwych sposobów na poprawê jakoœci regulacji jest pomiar zak³óceñ i uwzglêdnienie ich w sterowaniu.
...
Wartoœci sygna³ów wejœcia i wyjœcia procesu w punkcie pracy wynosz¹
...
Wykres potwierdza,¿e punkt pracy by³ wyznaczony poprawnie.
...
Wykres ten zosta³ przedstawiony na rys. \ref{Charakterystyka-statyczna}. Z wykresu wynika, ¿e zale¿noœæ jest liniowa, zatem wzmocnienie statyczne mo¿na wyznaczyæ metod¹ najmniejszych kwadratów.
...
Zmieniaj¹c wartoœci parametrów uda³o siê poprawiæ regulacjê, zarówno zmniejszy³a siê wartoœæ b³êdu, jak i wykresy wygl¹daj¹ lepiej. Zmnniejszy³y siê oscylacje wyjœcia procesu w momencie narastania oraz opadania sygna³u do wartoœci zadanej. Znacz¹co poprawi³ siê czas uzyskiwania wartoœci zadanej, nie zwiêkszaj¹c przy tym znacz¹co przesterowania.
...
Dobór parametrów dla regulatora DMC odbywa³ siê w podobny sposób.
W pierwszej kolejnoœci zosta³y wybrane parametry w sposób przypadkowy, nastêpnie parametry by³y lekko zmieniane.
Patrz¹c na wyniki symulacji mo¿na by³o wybraæ najlepszy nastaw. Eksperymenty DMC zosta³y przedstawione na rys. \ref{test-DMC1} - \ref{test-DMC6}.
Z przeprowadzonych eksperymentów wynika, ¿e dla parametrów: $ N = 20, N_\mathrm{u} = 3, \lambda = \num{1.5} $ wyniki regulacji s¹ najlepsze. Dla mniejszych wartoœci horyzontu predykcji i sterowania wynki znacz¹co siê pogarszaj¹, natomiast dla wiêkszych nie jest zauwa¿alna znacz¹ca zmiana.
...
W celu optymalizacji wskaŸnika b³êdu w zale¿noœci od parametrów obu regulatorów
zosta³ u¿yty algorytm generyczny, którego zadaniem by³o znalezienie minimum funkcji b³êdów.
...
Otrzymane nastawy nie ró¿ni¹ siê znacz¹co od tych uzyskanych metod¹ eksperymentaln¹, niemniej jednak b³êdy regulacji s¹ mniejsze.
Wykresy regulacji dla tych nastawów s¹ przedstawione na rys. \ref{PID-optymalizacja} i \ref{DMC-optymalizacja}.
Metoda optymalizacji jest bardziej przydatna dla regulatora PID ni¿ DMC, poniewa¿ znajduj¹c graniczne wartoœci N oraz $N_\mathrm{u}$ po ich dalszym zwiêkszaniu nie powoduje to znacz¹cej poprawy regulacji, zu¿ywa jednak wiêksze zasoby mocy obliczeniowej. Natomias dla regulatora PID znacz¹co oszczêdza czas potrzebny na testowanie zmian nastawów metodami eksperymentalymi.
